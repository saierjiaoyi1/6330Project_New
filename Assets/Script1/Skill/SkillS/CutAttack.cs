...
[CreateAssetMenu(fileName = "CutAttackSkill", menuName = "Skill/CutAttack", order = 2)]
public class CutAttackSkillSO : SkillSO
{
        ...
        foreach (SkillTargetInfo targetInfo in targets)
        {
            if (targetInfo.cell != null && targetInfo.cell.occupant != null)
            {
                ...
                if (validTarget)
                {
                    ...
                    {
                        targetInfo.cell.occupant.ReceiveDamage((int)finalValue, DamageType.Cut);
                    }
